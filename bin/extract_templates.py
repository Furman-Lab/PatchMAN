...
SEARCH_DB = '/vol/ek/share/databases/patchman_search_db/'
EXTRACT_DB = '/vol/ek/share/databases/patchman_extract_DB/'
...
def extract_templates_for_motif(matches, pepseq, plen, patch, receptor_pose, scrfxn, design):
    ...
    for match in matches:
        ...
        motif_pdb = match[1]
        motif_chain = match[2]
        motif_stretches = match[3]
        t = match[4]
        R = match[5]

        if not path.isfile(path.join(EXTRACT_DB, motif_pdb[1:3].lower(), motif_pdb.upper() + '.pdb')):
            pdb_pose = toolbox.pose_from_rcsb(motif_pdb)
        else:
            pdb_pose = pose_from_pdb(path.join(EXTRACT_DB, motif_pdb[1:3].lower(), motif_pdb.upper() + '.pdb')) # all pdbs are downloaded in the previous step (download_all_pdbs.sh)

        """What is the real chain of the structure from the search DB"""
        searchPDB_pose = pose_from_pdb(path.join(SEARCH_DB, motif_pdb[1:3].lower(),
                                                 motif_pdb + '_' + motif_chain + '.pdb'))
        searchPDB_chain = core.pose.get_chain_from_chain_id(1, searchPDB_pose)

        """Corresponding index of the chain in the strcuture from the extract DB"""
        chain_idx = core.pose.get_chain_id_from_chain(searchPDB_chain, pdb_pose)

        """Shift the indices by the first residue of the relevant chain"""
        chain_starts = pdb_pose.chain_begin(chain_idx)

        indices = [r + chain_starts for m_stretch in motif_stretches for r in m_stretch]  # the numbering in master output is from 0, so add at least 1
...
def thread_pepseq(complex_pose_name, complex_pose, pepseq, scrfxn):
    ...
    if fixbb_design(pep_sele, complex_pose_name, pepseq, scrfxn) == 1:
...
def create_complex(receptor, pose_to_cut, pep, complex_name):
    ...
    ########DEBUG:
    # pose_to_cut.dump_pdb('test_match.pdb')
    ##############
...
def parse_matches(match_file):
    ...
    for line in all_matches:
        ...
        if rmsd < 0.01: # skip identical matches
            continue
        pdb = line.split()[1].split('/')[-1].split('_')[0]
        chain = line.split()[1].split('/')[-1].split('_')[1].split('.')[0]
        stretches = []
        ...
        matches.append((rmsd, pdb, chain, stretches, t, u))
...
if __name__ == "__main__":
    ...
    init('-use_input_sc -ignore_unrecognized_res -ignore_zero_occupancy false -detect_disulf 0')  # for pyrosetta
