...
"""Receives a list of matches for *1* motif, the receptor pdbfile and the peptide sequence
Create potential complexes: extract peptides from proteins with motifs similar to the query,
 thread the pepseq with fixbb
 Take first 100 matches --> low RMSD, and last 100 matches --> depends on the rmsdCut"""


def extract_templates_for_motif(all_matches, pepseq, plen, patch, receptor_pose, scrfxn, design):
    ...
    print("%s matches were found for this motif"%str(len(all_matches[1:])))
    ...
    if len(all_matches) > OVERALL_MATCHES:
        matches = all_matches[:int(OVERALL_MATCHES/2)] + all_matches[-int(OVERALL_MATCHES/2):]
    else:
        matches = all_matches
...
def elongate_stretch(stretch, peplen, tot_res, chain_breaks): # TODO: add different chain filtering
...
def parse_matches(match_file):
    ...
    with open(match_file.strip(), 'r') as m:
        all_matches = m.readlines()

    matches = []
    for line in all_matches:  # first match is from the query itself; Some have an enormous amount of matches, need to limit somehow
